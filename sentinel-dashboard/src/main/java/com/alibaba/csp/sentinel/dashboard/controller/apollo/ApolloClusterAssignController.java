...
public class ApolloClusterAssignController  {

    private final Logger logger = LoggerFactory.getLogger(ApolloClusterAssignController.class);


    @Autowired
    @Qualifier("apolloClusterAppAssignPublisher")
    ...
    public Result<ClusterAppAssignResultVO> apiAssignSingleClusterServersOfApp(@PathVariable String app,
        ...
        try {
            rulePublisher.publish(app,Collections.singletonList(assignRequest.getClusterMap()));
            return Result.ofSuccess(new ClusterAppAssignResultVO());
